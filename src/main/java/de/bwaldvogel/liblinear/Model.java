 ...
 * 
 * <p>
 * use {@link Linear#loadModel(File)} and {@link Linear#saveModel(File, Model)}
 * to load/save it
 * </p>
 */
public final class Model implements Serializable {
	...
	/**
	 * The nr_feature*nr_class array w gives feature weights. We use one against
	 * the rest for multi-class classification, so each feature index
	 * corresponds to nr_class weight values. Weights are organized in the
	 * following way
	 * 
	 * <pre>
	 * +------------------+------------------+------------+
	 ...
	 * </pre>
	 * 
	 * If bias &gt;= 0, x becomes [x; bias]. The number of features is increased
	 * by one, so w is a (nr_feature+1)*nr_class array. The value of bias is
	 * stored in the variable bias.
	 * 
	 * @see #getBias()
	...
	public int hashCode() {
		...
		result = prime * result
				+ ((solverType == null) ? 0 : solverType.hashCode());
	...
	/**
	 * don't use {@link Arrays#equals(double[], double[])} here, cause 0.0 and
	 * -0.0 should be handled the same
	 * 
	 * @see Linear#saveModel(java.io.Writer, Model)
	 */
	protected boolean equals(double[] a, double[] a2) {
		if (a == a2)
			return true;
		if (a == null || a2 == null)
			return false;
