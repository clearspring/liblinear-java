...
class Tron {
	...
	void tron(double[] w) {
		...
		while (iter <= max_iter && search != 0) {
			...
			utils.info("iter %2d act %5.3e pre %5.3e delta %5.3e f %5.3e |g| %5.3e CG %3d%n",
			...
			if (f < -1.0e+32) {
				utils.info("warning: f < -1.0e+32%n");
				break;
			}
			if (Math.abs(actred) <= 0 && prered <= 0) {
				utils.info("warning: actred and prered <= 0%n");
				break;
			}
			if (Math.abs(actred) <= 1.0e-12 * Math.abs(f)
					&& Math.abs(prered) <= 1.0e-12 * Math.abs(f)) {
				utils.info("warning: actred and prered too small%n");
				break;
			}
		}
	}
	...
	private int trcg(double delta, double[] g, double[] s, double[] r) {
		...
		while (true) {
			...
			if (euclideanNorm(s) > delta) {
				utils.info("cg reaches trust region boundary%n");
				alpha = -alpha;
				daxpy(alpha, d, s);
	 ...
	 * @since 1.8
	 */
	private  void daxpy(double constant, double vector1[],
			double vector2[]) {
		if (constant == 0)
			return;
	 ...
	 * @since 1.8
	 */
	private  void scale(double constant, double vector[]) {
		if (constant == 1.0)
			return;
